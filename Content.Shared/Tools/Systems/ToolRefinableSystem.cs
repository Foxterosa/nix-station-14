...
public sealed partial class ToolRefinableSystem : EntitySystem
{
    ...
    private void OnDoAfter(Entity<ToolRefinableComponent> ent, ref ToolRefineDoAfterEvent args)
    {
        ...
        else
        {
            ...
            var spawned = SpawnRefinement(component.RefineResult, uid, rng);

            var refinedEv = new ToolRefinedEvent(spawned, args.User);
            RaiseLocalEvent(uid, ref refinedEv);
        }
    ...
    private List<EntityUid> SpawnRefinement(List<EntitySpawnEntry> spawnList, EntityUid source, System.Random rng)
    {
...
/// <summary>
/// Called after refining and spawning the results.
/// </summary>
[ByRefEvent]
public readonly record struct ToolRefinedEvent(List<EntityUid> SpawnedEntities, EntityUid User);

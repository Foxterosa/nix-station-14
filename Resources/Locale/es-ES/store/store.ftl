store-ui-default-title = Tienda
store-ui-default-withdraw-text = Retirar
store-ui-refund-text = Reembolsar
store-ui-balance-display = { $currency }: { $amount }
store-ui-price-display = { $amount } { $currency }
store-ui-discount-display-with-currency =  {$amount} de descuento en {$currency}
store-ui-discount-display =  ({$amount} de descuento)
store-ui-traitor-flavor = Derechos de autor © SCP-30643
store-ui-traitor-warning = Los operativos deben bloquear sus uplinks después de usarlos para evitar ser detectados.

store-withdraw-button-ui = Retirar {$currency}
store-ui-button-out-of-stock = {""} (Sin stock)
store-ui-button-charging = {""} (Cargando)
store-not-account-owner = No estás vinculado a {$store}.

store-preset-name-uplink = Enlace ascendente
store-preset-name-spellbook = Libro de hechizos
store-preset-name-nukie-delivery = Entrega bluespace de NukeOps
store-preset-name-changeling = Tienda de ADN
